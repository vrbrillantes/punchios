...
class ExpasionPanelsDemo extends StatefulWidget {
  ...
  @override
  _ExpansionPanelsDemoState createState() => new _ExpansionPanelsDemoState(questionlist: questionlist);
}

class _ExpansionPanelsDemoState extends State<ExpasionPanelsDemo> {
  List<DemoItem<dynamic>> _demoItems;
  ListView _listviewquestions;

  List<Question> questionlist = <Question>[];
  _ExpansionPanelsDemoState({Key key, this.questionlist});
//  List<Question> questionlist = <Question>[];
//  _ExpansionPanelsDemoState(this.questionlist);
  ...
  void initState() {
    ...
    _listviewquestions = new ListView.builder(
      padding: new EdgeInsets.all(8.0),
      itemCount: questionlist.length,
      itemBuilder: (_, int index) {
        return new EB(snapshot: questionlist[index]);
      },
    );
...
class EB extends StatelessWidget {
  EB({this.snapshot}); // modified
  final Question snapshot; // modified
//  final Animation animation;

  @override
  Widget build(BuildContext context) {
    return new Text(snapshot.question);
  }
}